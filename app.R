...
ui <- fluidPage(
  ...
  tags$style(HTML(paste(
    '#primaryTabs { background-image: url("img.png");}',
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      fluidRow(
        column(2, h3("Model Structure")),
        column(2, actionButton(inputId = "downloadDrawio", label = "Download drawio")),
        column(
          4,
          fileInput(inputId = "uploadDrawio", label = NULL, buttonLabel = "Upload drawio", multiple = FALSE, accept = ".drawio"),
...
server <- function(input, output, session) {
    ...
    vn %>%
      ...
      visOptions(
        highlightNearest = list(
          enabled = TRUE, degree = list(from = 0, to = 1),
  ...
  observeEvent(input$graphNodes, {
    ...
    edges <- map_df(
      ...
      function(edge) {
        tibble(
          from = nodeID_to_compartmentID[edge$from],
  ...
  observeEvent(input$uploadDrawio, {
    ...
    compartments <- dio$pages[[1]]$compartments |>
      ...
      transmute(
        label = as.character(compartmentNameSimple),
        x = x,
        y = y
      ) |>
      rowid_to_column(var = "id")
